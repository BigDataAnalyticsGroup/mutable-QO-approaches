		...
		\Function{split\_algebraic\_step}{G}
			...
			\State \CommentLeft{optimize join order by enumerating subproblem splits}
  	    	\For{l, r, cond $\in$ PE.enumerate(G)}\label{split-join_enum}
				...
				\State alg\_cost = AC.estimate(alg\_plan) + PT[l].cost + PT[r].cost
				\State s = alg\_plan.get\_subproblem() \Comment{unifies left and right subproblems}
				\If{alg\_cost < PT[s].cost}
					\State PT[s].plan = alg\_plan \Comment{update plan}
					\State PT[s].cost = alg\_cost \Comment{update cost}\label{split-join_end}
				\EndIf
  	  	   	\EndFor
		...
		\State \CommentLeft{Physically optimizes the algebraic operator. Inserts found QEPs into the plan table.}
	 	\Function{optimize\_physically}{PT, alg\_op}
  	 		\ForAll{\textit{registered physical operators} phys\_op}\label{physical-iterate_ops}
				...
				\For{p $\in$ permutate(s\_children)}\label{physical-iterate_permutations}
				\State \CommentLeft{iterate over Cartesian product of current permutation}
					\For{post\_conds, entries $\in$ PT[p[0]] $\times$ \dots $\times$ PT[p[n]]}\label{physical-iterate_children}
						...
						\State phys\_cost = PC.estimate(qep) + entries[0].cost + \dots + entries[n].cost
						\State s = qep.get\_subproblem() \Comment{unifies and adapts children subproblems}
						\If{phys\_cost < PT[s][post\_cond].cost}
							\State PT[s][post\_cond].plan = qep \Comment{update plan}
							\State PT[s][post\_cond].cost = phys\_cost \Comment{update cost}\label{physical-qep_end}
						\EndIf
					\EndFor
				\EndFor
  	 		\EndFor
...
\begin{table}[t]
	...
	\begin{subtable}[b]{\columnwidth}
		...
		\begin{tabular}{|c!{\vrule width .8pt}l!{\vrule width .8pt}c!{\vrule width .8pt}r|}
			\hline
			\underline{\smash{\textbf{Subproblem}}} & \underline{\smash{\textbf{Output Property}}} & \textbf{QEP} & \textbf{Cost} \\
			\noalign{\hrule height .8pt}
...
The idea to apply such a tree covering with certain conditions was inspired by the \emph{instruction selection problem} which is well-known in the compiler construction community~\cite{pattern_matching_in_trees, optimal_code_generation_for_expr_trees, code_generation_using_tree_matching_and_dp}.
...
When comparing \split and \holistic one can make an interesting observation.  \holistic can be rephrased as two-step optimization problem as follows.  Firstly, solve algebraic optimization by computing not only the best algebraic plan but all possible join orders.  Afterward, perform the physical optimization step for all algebraic plans received.  In practice, this strategy would surely increase the optimization time as both steps are no longer incorporated, however, it hints at the application of a hybrid approach, namely \topk.
