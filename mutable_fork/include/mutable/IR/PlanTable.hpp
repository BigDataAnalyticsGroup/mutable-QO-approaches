...
struct M_EXPORT PlanTableEntry
{
    ...
    ///> additional data associated to this `PlanTableEntry`; used for top-k and holistic optimization
...
struct M_EXPORT PlanTableBase : crtp<Actual, PlanTableBase>
{
    ...
    friend void swap(PlanTableBase &first, PlanTableBase &second) { swap(first.actual(), second.actual()); }
...
M_LCOV_EXCL_START
    public:
    friend std::ostream & operator<<(std::ostream &out, const PlanTableBase &PT) { return out << PT.actual(); }
