...
struct enumerate_tag : const_virtual_crtp_helper<enumerate_tag>::
    ...
    crtp_args<PlanTableSmallOrDense&, PlanTableLargeAndSparse&,
              TopKPlanTable<PlanTableSmallOrDense, 2>&,  TopKPlanTable<PlanTableLargeAndSparse, 2>&,
              TopKPlanTable<PlanTableSmallOrDense, 3>&,  TopKPlanTable<PlanTableLargeAndSparse, 3>&,
              TopKPlanTable<PlanTableSmallOrDense, 5>&,  TopKPlanTable<PlanTableLargeAndSparse, 5>&,
              TopKPlanTable<PlanTableSmallOrDense, 10>&, TopKPlanTable<PlanTableLargeAndSparse, 10>&,
              TopKPlanTable<PlanTableSmallOrDense, 34>&, TopKPlanTable<PlanTableLargeAndSparse, 34>&,
              TopKPlanTable<PlanTableSmallOrDense, 35>&, TopKPlanTable<PlanTableLargeAndSparse, 35>&,
              HolisticPlanTable<PlanTableSmallOrDense>&, HolisticPlanTable<PlanTableLargeAndSparse>&>::
...
/** Computes the join order by enumerating *all* join orders, including Cartesian products.  Stores its current state
 * s.t. it updates the given plan table to represent the next best plan in consecutive calls, e.g. first call gives
 * the best plan, second call gives the second best plan, etc. */
struct M_EXPORT PEexhaustive final : PlanEnumeratorCRTP<PEexhaustive>
{
    using base_type = PlanEnumeratorCRTP<PEexhaustive>;
    using base_type::operator();

    private:
    using plan_instance_t = std::vector<Subproblem>; ///< one plan instance consists of all used subproblem splits
    mutable std::vector<plan_instance_t> plan_instances_; ///< pre-computed plan instance
    mutable const QueryGraph *G_ = nullptr;
