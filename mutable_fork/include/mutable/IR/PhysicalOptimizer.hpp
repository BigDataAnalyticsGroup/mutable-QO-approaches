...
struct MatchBase
{
    ...
    friend std::pair<std::unique_ptr<Operator>, std::unique_ptr<MatchBase>> reconstruct_logical_plan(const MatchBase&);
...
struct PhysicalOptimizer
{
    ...
    virtual ~PhysicalOptimizer() {
        if (Options::Get().statistics) {
            std::cout << "Number of enumerated physical plans: " << num_instantiations << std::endl;
            num_instantiations = 0; // reset to 0
        }
    }
...
struct PhysicalOptimizerImpl : PhysicalOptimizer, ConstPostOrderOperatorVisitor
{
    ...
    explicit PhysicalOptimizerImpl(PhysicalPlanTable &&table) : table_(std::move(table)) { }
    ...
    void handle_match(const Operator &op, std::unique_ptr<Match<PhysOp>> &&match, const children_type &children) {
        ...
        auto it_implied = [&](){
            using iterator = typename PhysicalPlanTable::condition2entry_map_type::iterator;
            const auto end = table()[op.id()].end();
            auto find_implied_rec = [&post_cond, &end](iterator curr_res, iterator curr_it, auto &rec){
                auto res = std::find_if(curr_it, end, [&post_cond, &curr_res, &end](const auto &e) {
                    return e.condition.implied_by(post_cond) and // implied by `post_cond`
                           (curr_res == end or curr_res->condition.implied_by(e.condition)); // "bigger" than `curr_res`
                });
                if (res == end)
                    return curr_res;
                else
                    return rec(res, std::next(res), rec); // recursively search for "bigger" implied conditions
            };
            return find_implied_rec(end, table()[op.id()].begin(), find_implied_rec);
        }();
        ...
        if (it_implied == table()[op.id()].end()) {
            ...
            if (cost < it_implied->entry.cost()) {
                it_implied->entry = entry_type(std::move(match), children, cost);
                it_implied->condition = std::move(post_cond); // upgrade condition to "bigger" condition
            } else {
                if (it_equal == table()[op.id()].end()) {
                    table()[op.id()].insert(std::move(post_cond), entry_type(std::move(match), children, cost));
                } else {
                    if (cost < it_equal->entry.cost())
                        it_equal->entry = entry_type(std::move(match), children, cost);
                    else
...
#define M_PHYS_OPT_LIST(X) \
    ...
    X(PhysicalOptimizerImpl<HolisticPlanTableLargeAndSparse>) \
    X(PhysicalOptimizerImpl<HolisticPlanTableSmallOrDense>)
