...
M_LCOV_EXCL_STOP
    ...
    struct entry_type
    {
        enum constraints_t : uint64_t
        {
            ...
            IS_HIDDEN = 0b100U,     ///< entry is hidden to the user
        };

        Identifier id;
        const Type *type;
        constraints_t constraints;
        std::optional<Identifier> determined_by; ///< part of constraints; optionally functionally determining identifier
        ...
        entry_type(Identifier id, const Type *type, constraints_t constraints = constraints_t{0},
                   std::optional<Identifier> determined_by = std::optional<Identifier>{})
            : id(std::move(id))
            , type(M_notnull(type))
            , constraints(constraints)
            , determined_by(std::move(determined_by))
        { }
        ...
        bool is_determined() const { return bool(determined_by); }
        bool is_hidden() const { return bool(IS_HIDDEN & constraints); }
    };
    ...
    /** Adds a new entry `id` of type `type` with constraints `constraints` and optional determining identifier
     * `determined_by` to this `Schema`. */
    void add(Identifier id, const Type *type, entry_type::constraints_t constraints = entry_type::constraints_t{0},
             std::optional<Identifier> determined_by = std::optional<Identifier>{})
    {
        entries_.emplace_back(std::move(id), type, constraints, std::move(determined_by));
    }
    ...
    Schema deduplicate() const {
        ...
        for (auto &e : *this) {
            if (not res.has(e.id))
                res.add(e.id, e.type, e.constraints, e.determined_by);
        }
        return res;
    }
...
inline Schema operator&(const Schema &left, const Schema &right)
{
    ...
    for (auto &e : left) {
        ...
        if (it != right.end()) {
            ...
            if (e.is_determined() and it->is_determined() and e.determined_by != it->determined_by)
                throw invalid_argument("constraint mismatch (determined by)");
            auto determined_by = e.is_determined() ? e.determined_by : it->determined_by;
            res.add(e.id, e.type, e.constraints | it->constraints, determined_by); // merge constraints from both
        }
    }
    return res;
}
