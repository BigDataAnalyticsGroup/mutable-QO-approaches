...
class Mutable(Connector):
    ...
    MUTABLE_TYPE_PARSER: dict[str, Callable[[list[str]], str]] = {
        ...
        'VARCHAR':     lambda ty: f'VARCHAR({ty[1]})',
        'DATE':        lambda ty: 'DATE',
...
#=======================================================================================================================
    def run_configuration(self, n_runs: int, config_name: str, config: dict[str, Any], yml: dict[str, Any]) -> ConfigResult:
        ...
        execute_multiple: bool = '--xxx-optimizer-exhaustive' in command
        ...
        if False and not self.check_execute_single_cases(yml):
            ...
            try:
                ...
                if binargs and '--dryrun' in binargs:
                    tqdm_print(out)
                ...
                for label, durations in zip(pattern_labels, measurements):
                    if execute_multiple:
                        if len(durations) % (n_runs * len(cases)) != 0:
                            raise ConnectorException(f"Expected multiple of {n_runs * len(cases)} measurements per pattern "
                                                     f"but got {len(durations)} for pattern label '{label}'.")
                        # Add measured times
                        num_multiple: int = int(len(durations) / (n_runs * len(cases)))
                        for i in range(n_runs):
                            run_durations: list[float] = durations[i * len(cases) * num_multiple : (i+1) * len(cases) * num_multiple]
                            for j, case in enumerate(cases.keys()):
                                case_durations: list[float] = run_durations[j * num_multiple : (j+1) * num_multiple]
                                for k, dur in enumerate(case_durations):
                                    case_str: str = case + str(k)
                                    config_result[label].setdefault(case_str, list()).append(dur)
                    else:
