...
#define ADD(TYPE, VAR, INIT, SHORT, LONG, DESCR, CALLBACK)\
    ...
    /*----- Select type of query optimizer to use --------------------------------------------------------------------*/
    ADD(bool, Options::Get().optimizer_type, Options::Opt_auto,                   /* Type, Var, Init  */
        nullptr, "--optimizer-split",                                             /* Short, Long      */
        "use the split query optimizer, i.e. independent "
        "logical and physical optimization steps",                                /* Description      */
        [&](bool) { Options::Get().optimizer_type = Options::Opt_Split; });       /* Callback         */
    ADD(bool, Options::Get().optimizer_type, Options::Opt_auto,                   /* Type, Var, Init  */
        nullptr, "--optimizer-holistic",                                          /* Short, Long      */
        "use the holistic query optimizer, i.e. incorporating "
        "logical and physical optimization into a single task",                   /* Description      */
        [&](bool) { Options::Get().optimizer_type = Options::Opt_Holistic; });    /* Callback         */
    ADD(std::size_t, Options::Get().optimizer_type, Options::Opt_auto,            /* Type, Var, Init  */
        nullptr, "--optimizer-topk",                                              /* Short, Long      */
        "use the top-k query optimizer, i.e. enumerating the best k logical "
        "plans (using the split approach, i.e. the best physical "
        "plan per enumerated logical plan) and specify the hyper-parameter k",    /* Description      */
        [&](std::size_t k) {
    ...
    ADD(bool, Options::Get().optimizer_type, Options::Opt_auto,                   /* Type, Var, Init  */
        ...
        "all possible plans (using the split approach, i.e. the best physical "
        "plan per enumerated logical plan)",                                      /* Description      */
        [&](bool) { Options::Get().optimizer_type = Options::Opt_Exhaustive; });  /* Callback         */
    ...
    ADD(bool, Options::Get().list_cost_functions, false,    /* Type, Var, Init  */
        ...
        "list all available logical cost functions",        /* Description      */
        [&](bool) {                                         /* Callback         */
