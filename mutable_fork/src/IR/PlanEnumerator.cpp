...
/*======================================================================================================================
 * PEexhaustive (only for exhaustive optimizer, not applicable independently!)
 *====================================================================================================================*/

template<typename PlanTable>
inline void PEexhaustive::operator()(enumerate_tag, PlanTable &PT, const QueryGraph &G, const CostFunction &CF) const
{
    const std::size_t n = G.sources().size();
    auto &CE = Catalog::Get().get_database_in_use().cardinality_estimator();

    auto pre_compute = [this](Subproblem S){
        auto pre_compute_impl = [](Subproblem S, auto &rec) -> std::vector<plan_instance_t> {
            const std::size_t n = S.size();
            if (n == 1)
                return {{}}; // s.t. Cartesian product computation iterates at least once to add subproblems

            std::vector<plan_instance_t> res;
            const std::size_t m = n / 2; // division with rounding down
            for (std::size_t s1 = 1; s1 <= m; ++s1) {
                const std::size_t s2 = n - s1;
                const auto hi = S.hi(); // to prune second half, i.e. hi bit is reached, of subset enumeration if symmetric sizes
                for (auto S1 = SubsetEnumerator(S, s1); s1 == s2 ? S1 and not (*S1 & hi) : S1; ++S1) { // enumerate all subsets of size s1
                    if (not (*S1 & S)) continue; // S1 not part of S -> skip
                    const auto S2 = S - *S1; // S2 is complement of S1 regarding S
                    const auto res1 = rec(*S1, rec);
                    const auto res2 = rec(S2, rec);
                    for (auto &plan1 : res1) {
                        for (auto &plan2 : res2) {
                            auto &plan = res.emplace_back(plan_instance_t{ *S1, S2 });
                            plan.insert(plan.end(), plan1.begin(), plan1.end()); // copy for Cartesian product
                            plan.insert(plan.end(), plan2.begin(), plan2.end()); // copy for Cartesian product
                        }
                    }
                }
            }
        ...
        plan_instances_ = pre_compute_impl(S, pre_compute_impl);
    };
    if (not G_) {
        G_ = &G; // set graph on first call
        pre_compute(Subproblem::All(n));
    }
    M_insist(G_ == &G, "must not change query graph between consecutive calls");

    if (plan_instances_.empty()) {
        G_ = nullptr; // reset graph on last call
        return;
    }

    auto &plan_instance = plan_instances_.back();
    M_insist(plan_instance.size() % 2 == 0);
    for (auto it = plan_instance.crbegin(); it != plan_instance.crend(); ++it) {
        cnf::CNF condition; // TODO use join condition
        PT.update(G, CE, CF, *it, *(++it), condition);
    }
    plan_instances_.pop_back();
}
...
#define LIST_PE(X) \
    ...
    X(PEall,        "enumerates ALL join orders, including Cartesian products") \
    X(PEexhaustive, "independently enumerates and stores ALL join orders, including Cartesian products, should only be used for exhaustive optimization")
...
#define INSTANTIATE(NAME, _) \
    ...
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableSmallOrDense, 2> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableLargeAndSparse, 2> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableSmallOrDense, 3> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableLargeAndSparse, 3> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableSmallOrDense, 5> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableLargeAndSparse, 5> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableSmallOrDense, 10> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableLargeAndSparse, 10> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableSmallOrDense, 34> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableLargeAndSparse, 34> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableSmallOrDense, 35> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, TopKPlanTable<PlanTableLargeAndSparse, 35> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, HolisticPlanTable<PlanTableSmallOrDense> &PT, const QueryGraph &G, const CostFunction &CF) const; \
    template void NAME::operator()(enumerate_tag, HolisticPlanTable<PlanTableLargeAndSparse> &PT, const QueryGraph &G, const CostFunction &CF) const;
LIST_PE(INSTANTIATE)
#undef INSTANTIATE
