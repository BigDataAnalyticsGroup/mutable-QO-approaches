...
std::pair<std::unique_ptr<Producer>, PlanTable> Optimizer::optimize_with_plantable(QueryGraph &G) const
{
    ...
    if (not PT.has_plan(Subproblem::All(num_sources))) {
        for (auto &ds : G.sources())
            delete source_plans[ds->id()];
        return { nullptr, std::move(PT) }; // for exhaustive PE
    }
...
void Optimizer::optimize_join_order(const QueryGraph &G, PlanTable &PT) const
{
    ...
    if (Options::Get().statistics and PT.has_plan(Subproblem::All(G.num_sources()))) {
...
void \
Optimizer::optimize_join_order(const QueryGraph&, PLANTABLE&) const;
DEFINE(TopKPlanTable<M_COMMA(PlanTableSmallOrDense) 2>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableLargeAndSparse) 2>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableSmallOrDense) 3>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableLargeAndSparse) 3>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableSmallOrDense) 5>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableLargeAndSparse) 5>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableSmallOrDense) 10>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableLargeAndSparse) 10>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableSmallOrDense) 34>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableLargeAndSparse) 34>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableSmallOrDense) 35>);
DEFINE(TopKPlanTable<M_COMMA(PlanTableLargeAndSparse) 35>);
#undef DEFINE
