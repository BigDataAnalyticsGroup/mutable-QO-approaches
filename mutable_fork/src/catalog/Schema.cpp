...
Schema ConcreteTable::schema(const ThreadSafePooledOptionalString &alias) const
{
    ...
    for (auto attr = this->begin_all(); attr != this->end_all(); ++attr) {
        ...
        if (key.size() == 1 and key[0].get() != *attr)
            singleton_primary_key.emplace(alias.has_value() ? alias : this->name(), key[0].get().name);
        S.add({alias.has_value() ? alias : this->name(), attr->name}, attr->type, constraints, std::move(singleton_primary_key));
    }
    return S;
}
