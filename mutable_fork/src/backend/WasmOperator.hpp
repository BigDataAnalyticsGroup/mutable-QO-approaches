...
/** Which index implementations should be considered for `wasm::ISAM` or `wasm::IndexScan`. */
inline option_configs::IndexImplementation index_implementations = option_configs::IndexImplementation::ARRAY;

/** Which index scan strategy should be used for `wasm::ISAM` or `wasm::IndexScan`. */
inline option_configs::IndexScanStrategy index_scan_strategy = option_configs::IndexScanStrategy::INTERPRETATION;

/** Which compilation strategy should be used for `wasm::ISAM` or `wasm::IndexScan`. */
...
/** Which materialization strategy should be used for `wasm::ISAM` or `wasm::IndexScan`. */
...
#define M_WASM_OPERATOR_LIST_TEMPLATED(X) \
    ...
    X(Scan<false>) \
    X(Scan<true>) \
    X(ISAM<M_COMMA(m::idx::IndexMethod::Array) 0>) \
    X(ISAM<M_COMMA(m::idx::IndexMethod::Array) 1>) \
    X(ISAM<M_COMMA(m::idx::IndexMethod::Array) 2>) \
    X(ISAM<M_COMMA(m::idx::IndexMethod::Array) 3>) \
    X(ISAM<M_COMMA(m::idx::IndexMethod::Array) 4>) \
    X(ISAM<M_COMMA(m::idx::IndexMethod::Array) 5>) \
    X(ISAM<M_COMMA(m::idx::IndexMethod::Array) 6>) \
    X(ISAM<M_COMMA(m::idx::IndexMethod::Array) 7>) \
    X(ISAM<M_COMMA(m::idx::IndexMethod::Array) 8>) \
...
#define M_WASM_MATCH_LIST_TEMPLATED(X) \
    ...
    X(m::Match<m::wasm::ISAM<M_COMMA(m::idx::IndexMethod::Array) 0>>) \
    X(m::Match<m::wasm::ISAM<M_COMMA(m::idx::IndexMethod::Array) 1>>) \
    X(m::Match<m::wasm::ISAM<M_COMMA(m::idx::IndexMethod::Array) 2>>) \
    X(m::Match<m::wasm::ISAM<M_COMMA(m::idx::IndexMethod::Array) 3>>) \
    X(m::Match<m::wasm::ISAM<M_COMMA(m::idx::IndexMethod::Array) 4>>) \
    X(m::Match<m::wasm::ISAM<M_COMMA(m::idx::IndexMethod::Array) 5>>) \
    X(m::Match<m::wasm::ISAM<M_COMMA(m::idx::IndexMethod::Array) 6>>) \
    X(m::Match<m::wasm::ISAM<M_COMMA(m::idx::IndexMethod::Array) 7>>) \
    X(m::Match<m::wasm::ISAM<M_COMMA(m::idx::IndexMethod::Array) 8>>) \
...
namespace wasm { template<idx::IndexMethod IndexMethod, std::size_t AttrIdx> struct ISAM; }
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx> struct Match<wasm::ISAM<IndexMethod, AttrIdx>>;
...
struct ISAM : PhysicalOperator<ISAM<IndexMethod, AttrIdx>, ScanOperator>
{
    static void execute(const Match<ISAM> &M, setup_t setup, pipeline_t pipeline, teardown_t teardown);
    static double cost(const Match<ISAM> &M);
    static ConditionSet pre_condition(std::size_t child_idx,
                                      const std::tuple<const ScanOperator*> &partial_inner_nodes);
    static ConditionSet post_condition(const Match<ISAM> &M);
};
...
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx>
struct Match<wasm::ISAM<IndexMethod, AttrIdx>> : wasm::MatchLeaf
{
    const ScanOperator &scan;
    ...
    void execute(setup_t setup, pipeline_t pipeline, teardown_t teardown) const override {
        execute_buffered(*this, scan.schema(), buffer_factory_, buffer_num_tuples_,
                         std::move(setup), std::move(pipeline), std::move(teardown));
    }
