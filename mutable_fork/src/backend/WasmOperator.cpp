...
void m::register_wasm_operators(PhysicalOptimizer &phys_opt)
{
    ...
    if (bool(options::scan_implementations bitand option_configs::ScanImplementation::INDEX_SCAN)) {
        if (bool(options::index_implementations bitand option_configs::IndexImplementation::ARRAY)) {
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 0>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 1>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 2>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 3>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 4>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 5>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 6>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 7>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 8>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 9>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 10>>();
            phys_opt.register_operator<ISAM<idx::IndexMethod::Array, 11>>();
        ...
        if (bool(options::index_implementations bitand option_configs::IndexImplementation::RMI)) {
            M_unreachable("invalid index implementation");
        }
    }
...
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx>
ConditionSet ISAM<IndexMethod, AttrIdx>::pre_condition(std::size_t child_idx,
                                                       const std::tuple<const ScanOperator*> &partial_inner_nodes)
{
    M_insist(child_idx == 0);

    /*----- Check if index on the `AttrIdx`-th attribute in scan schema exists. -----*/
    auto &scan = *std::get<0>(partial_inner_nodes);
    auto &table = scan.store().table();
    ...
    if (AttrIdx >= table.num_attrs())
        return ConditionSet::Make_Unsatisfiable();

    Schema::Identifier id(scan.alias(), table[AttrIdx].name);
    if (not scan.schema().has(id))
        return ConditionSet::Make_Unsatisfiable();

    if (not DB.has_index(table.name(), id.name, IndexMethod))
        return ConditionSet::Make_Unsatisfiable();
...
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx>
ConditionSet ISAM<IndexMethod, AttrIdx>::post_condition(const Match<ISAM<IndexMethod, AttrIdx>> &M)
{
    ConditionSet post_cond;
    ...
    Sortedness::order_t orders;
    Schema::Identifier id(M.scan.alias(), M.scan.store().table()[AttrIdx].name);
...
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx>
double ISAM<IndexMethod, AttrIdx>::cost(const Match<ISAM<IndexMethod, AttrIdx>>&)
{
    /* TODO: add meaningful cost function. */
    return 1.1;
}

template<idx::IndexMethod IndexMethod, std::size_t AttrIdx, typename Index, sql_type SqlT>
void isam_codegen_compilation(const Index &index, const Match<ISAM<IndexMethod, AttrIdx>> &M,
...
#undef SET_CALLBACK_FNS
        ...
        const uint64_t alloc_size([&](){
            uint64_t num_results = index.num_entries();
            uint64_t batch_size = M.batch_size == 0 ? num_results : M.batch_size;
            return batch_size < num_results ? batch_size : num_results;
        }());
        Ptr<U64x1> buffer_address = Module::Allocator().pre_malloc<uint64_t>(alloc_size);

        /*----- Emit setup code *after* allocating memory to guarantee sequential memory allocation for pipeline. -----*/
        setup();

        /*----- Emit loop code. -----*/
        M_insist(std::in_range<uint64_t>(index.num_entries()), "should fit in uint64_t");
        Var<U64x1> lo(0);
        Var<U64x1> hi(index.num_entries());
...
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx, typename Index>
void isam_codegen_interpretation(const Index &index, const Match<ISAM<IndexMethod, AttrIdx>> &M,
                                 setup_t setup, pipeline_t pipeline, teardown_t teardown)
{
    ...
    if (options::index_scan_materialization_strategy == option_configs::IndexScanMaterializationStrategy::MEMORY) {
        ...
        /*----- Perform index sequential scan and fill memory with results. -----*/
        ...
        FUNCTION(isam_parent_pipeline, void(uint64_t))
        {
            auto S = CodeGenContext::Get().scoped_environment(); // create scoped environment for this function
        ...
        for (auto it = index.begin(); it != index.end(); ++it) {
            M_insist(std::in_range<uint64_t>(it->second), "tuple id must fit in uint64_t");
            isam_parent_pipeline(uint64_t(it->second));
        }
    } else {
...
/** Resolves the index sequential access strategy and calls the appropriate codegen function. */
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx, typename Index, sql_type SqlT>
void isam_resolve_strategy(const Index &index, const Match<ISAM<IndexMethod, AttrIdx>> &M,
                           setup_t setup, pipeline_t pipeline, teardown_t teardown)
{
    if (options::index_scan_strategy == option_configs::IndexScanStrategy::COMPILATION) {
        isam_codegen_compilation<IndexMethod, AttrIdx, Index, SqlT>(index, M, std::move(setup), std::move(pipeline), std::move(teardown));
    } else if (options::index_scan_strategy == option_configs::IndexScanStrategy::INTERPRETATION) {
        isam_codegen_interpretation<IndexMethod, AttrIdx, Index>(index, M, std::move(setup), std::move(pipeline), std::move(teardown));
    } else {
...
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx, typename AttrT, sql_type SqlT>
void isam_resolve_index_method(const Schema::Identifier &id, const Match<ISAM<IndexMethod, AttrIdx>> &M,
                               setup_t setup, pipeline_t pipeline, teardown_t teardown)
{
    /*----- Lookup index. -----*/
    Catalog &C = Catalog::Get();
    ...
    auto &index_base = DB.get_index(table_name, id.name, IndexMethod);
    ...
    if constexpr(IndexMethod == idx::IndexMethod::Array and requires { typename idx::ArrayIndex<AttrT>; }) {
        ...
        isam_resolve_strategy<IndexMethod, AttrIdx, const idx::ArrayIndex<AttrT>, SqlT>(
            index, M, std::move(setup), std::move(pipeline), std::move(teardown)
        );
        ...
        isam_resolve_strategy<IndexMethod, AttrIdx, const idx::RecursiveModelIndex<AttrT>, SqlT>(
            index, M, std::move(setup), std::move(pipeline), std::move(teardown)
        );
    } else {
...
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx>
void isam_resolve_attribute_type(const Match<ISAM<IndexMethod, AttrIdx>> &M,
                                 setup_t setup, pipeline_t pipeline, teardown_t teardown)
{
    auto &attr = M.scan.store().table()[AttrIdx];
    Schema::Identifier id(M.scan.alias(), attr.name);

    /*----- Resolve attribute type. -----*/
#define RESOLVE_INDEX_METHOD(ATTRTYPE, SQLTYPE) \
    isam_resolve_index_method<IndexMethod, AttrIdx, ATTRTYPE, SQLTYPE>(id, M, std::move(setup), std::move(pipeline), std::move(teardown))
...
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx>
void ISAM<IndexMethod, AttrIdx>::execute(const Match<ISAM<IndexMethod, AttrIdx>> &M,
                                              setup_t setup, pipeline_t pipeline, teardown_t teardown)
{
    auto &schema = M.scan.schema();
    ...
    auto &table = M.scan.store().table();
    M_insist(not table.layout().is_finite(), "layout for `wasm::ISAM` must be infinite");

    /** Generating code for an index sequential access requires resolving the following:
    ...
    isam_resolve_attribute_type(M, std::move(setup), std::move(pipeline), std::move(teardown));
}
...
{
    if (M.build.info().estimated_cardinality == 0 or M.probe.info().estimated_cardinality == 0)
        return -0.00002;
    ...
    if (options::simple_hash_join_ordering_strategy == option_configs::OrderingStrategy::BUILD_ON_LEFT)
        ...
        return 1.5 * M.build.info().estimated_cardinality + M.probe.info().estimated_cardinality;
}
...
double SortMergeJoin<SortLeft, SortRight, Predicated, CmpPredicated>::cost(const Match<SortMergeJoin> &M)
{
    ...
    if (card_left == 0 or card_right == 0)
        return -0.00001;
        ...
        &partial_inner_nodes)
{
    ...
    auto uniquely_determined = [&join](const std::vector<Schema::Identifier> &haystack, const Schema::Identifier &needle) {
        const auto &e = join.schema()[needle].second;
        return e.is_determined() and contains(haystack, *e.determined_by);
    };
    ...
    if (child_idx == 1) {
        ...
        const auto [build_keys, probe_keys] = decompose_equi_predicate(join.predicate(), build.schema());
        const auto num_join_keys = build_keys.size(); // equals probe_keys.size()
        auto combined_keys = std::move(build_keys);
        combined_keys.insert(combined_keys.end(), std::make_move_iterator(probe_keys.begin()),
                             std::make_move_iterator(probe_keys.end())); // merge build and probe keys

        /*----- Hash-based group-join can only be used if grouping and join (i.e. build) key match (ignoring order and uniquely determined grouping keys). -----*/
        const auto num_grouping_keys = grouping.group_by().size();
        if (num_grouping_keys < num_join_keys) // XXX: duplicated IDs may still be a match but are rejected here
            return ConditionSet::Make_Unsatisfiable();
        for (std::size_t i = 0; i < num_grouping_keys; ++i) {
            Schema::Identifier grouping_key(grouping.group_by()[i].first.get());
            if (not contains(combined_keys, grouping_key) and not uniquely_determined(combined_keys, grouping_key))
                return ConditionSet::Make_Unsatisfiable();
        }
    }
                                 ...
                                 teardown_t teardown)
{
    ...
    /*----- Compute key indices of hash table and uniquely determined keys (which are *not* hash table keys). -----*/
    std::vector<HashTable::index_t> key_indices;
    ///> map hash table schema indices to uniquely determined keys (due to mismatch of identifiers before and after grouping)
    std::vector<std::pair<std::size_t, Schema::Identifier>> uniquely_determined_build_keys, uniquely_determined_probe_keys;
    auto uniquely_determined = [&M](const std::vector<Schema::Identifier> &haystack, const Schema::Identifier &needle) {
        const auto &e = M.join.schema()[needle].second;
        return e.is_determined() and contains(haystack, *e.determined_by);
    };
    for (std::size_t i = 0; i < num_keys; ++i) {
        Schema::Identifier grouping_key(M.grouping.group_by()[i].first.get());
        if (contains(build_keys, grouping_key) or contains(probe_keys, grouping_key)) {
            key_indices.push_back(i);
        } else if (uniquely_determined(build_keys, grouping_key)) {
            uniquely_determined_build_keys.emplace_back(i, std::move(grouping_key));
        } else {
            M_insist(uniquely_determined(probe_keys, grouping_key));
            uniquely_determined_probe_keys.emplace_back(i, std::move(grouping_key));
        }
    }
    M_insist(build_keys.size() + uniquely_determined_build_keys.size() + uniquely_determined_probe_keys.size() == num_keys);
            ...
            /* pipeline= */ [&](){
                ...
                IF (*build_key_not_null) { // TODO: predicated version
                    ...
                    IF (inserted) {
                        ...
                        for (auto &[idx, id] : uniquely_determined_build_keys) {
                            std::visit(overloaded {
                                [&]<sql_type T>(HashTable::reference_t<T> &&r) -> void {
                                    r = env.get<T>(id); // set uniquely determined key
                                },
                                [](std::monostate&&) -> void { M_unreachable("invalid reference"); },
                            }, entry.extract(ht_schema[idx].id));
                        }
                    } ELSE {
                        update_aggs.attach_to_current();
                        update_avg_aggs.attach_to_current(); // after others to ensure that running count is incremented before
    ...
    FUNCTION(hash_based_group_join_probe_child_pipeline, void(void)) // create function for pipeline
    {
        ...
        M.children[1]->execute(
            ...
            /* pipeline= */ [&](){
                ...
                /*----- If group has been found, update aggregates. -----*/
                M_insist(init_aggs.empty(), "aggregates must be initialized in build phase");
                IF (found) {
                    update_aggs.attach_to_current();
                    update_avg_aggs.attach_to_current(); // after others to ensure that running count is incremented before

                    for (auto &[idx, id] : uniquely_determined_probe_keys) {
                        std::visit(overloaded {
                            [&]<sql_type T>(HashTable::reference_t<T> &&r) -> void {
                                r = env.get<T>(id); // set uniquely determined key XXX: probe counter could be checked to set only on first visit
                            },
                            [](std::monostate&&) -> void { M_unreachable("invalid reference"); },
                        }, entry.extract(ht_schema[idx].id));
                    }
...
template<idx::IndexMethod IndexMethod, std::size_t AttrIdx>
void Match<m::wasm::ISAM<IndexMethod, AttrIdx>>::print(std::ostream &out, unsigned level) const
{
    if (IndexMethod == idx::IndexMethod::Array)
        indent(out, level) << "wasm::ArrayISAM(";
    else if (IndexMethod == idx::IndexMethod::Rmi)
        indent(out, level) << "wasm::RecursiveModelISAM(";
    else
    ...
    Schema::Identifier id(this->scan.alias(), this->scan.store().table()[AttrIdx].name);
    out << "], " << this->scan.alias() << ", " << id << ") ";
    if (this->buffer_factory_ and this->scan.schema().drop_constants().deduplicate().num_entries())
        out << "with " << this->buffer_num_tuples_ << " tuples output buffer ";
    out << this->scan.schema() << print_info(this->scan) << " (cumulative cost " << cost() << ')';
}
